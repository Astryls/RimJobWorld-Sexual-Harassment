BODY tattoo / brand art intake
==============================
Drop BODY-tattoo / brand DDS files here (everything you listed as NOT face).

Naming — RimWorld renders tattoos as a Graphic_Multi, so each mark is a
...
- BODY tattoos are masked to the pawn's NAKED BODY graphic
  (bodyType.bodyNakedGraphicPath) and tinted through the skin-overlay shader, so
  the SAME 3 files auto-fit Male/Female/Thin/Fat/Hulk — you do NOT need per-body
  variants. But the art must sit on the vanilla body canvas so the mask lines up.
...
- This folder (_ArtIntake) is OUTSIDE Textures/, so RimWorld ignores it. Once a
  set is correct I move it to Textures/RJWSH/Tattoos/Body/ and add the TattooDef.
